...
class Finger:
    ...
    def visualCtrl(self,goal_x,goal_y, goal_theta):
        ...
        while norm(e) > TOLERANCE and i  < MAX_ITERATIONS:
            ...
            X = np.array([self.x, self.y])
            self.ik_rightFinger()
            J_right = np.matrix([[-(self.d2 + self.w0 / 2.0) * sin(self.t2) - (self.w0 / 2 + self.fw) * cos(self.t2)], [(self.d2 + self.w0 / 2) * cos(self.t2) - (self.w0 / 2 + self.fw) * sin(self.t2)]], dtype='float')
            self.ik_leftFinger()
            J_left = np.matrix([[-(self.d1 + self.w0 / 2.0) * sin(self.t1) + (self.w0 / 2 + self.fw) * cos(self.t1)], [(self.d1 + self.w0 / 2.0) * cos(self.t1) + (self.w0 / 2 + self.fw) * sin(self.t1)]], dtype='float')
            dtheta1 = 0.1
            dtheta2 = -0.1
            pos[:,0] = X + np.reshape(J_left*dtheta1*dt,(2))
            pos[:,1] = X + np.reshape(J_left*dtheta2*dt,(2))
            pos[:,2] = X + np.reshape(J_right*dtheta1*dt,(2))
            pos[:,3] = X + np.reshape(J_right*dtheta2*dt,(2))
            print errors[1]
            errors[0] = norm(self.X_d - pos[:,0])
            errors[1] = norm(self.X_d - pos[:,1])
            errors[2] = norm(self.X_d - pos[:,2])
            errors[3] = norm(self.X_d - pos[:,3])
            print errors
            min_index = np.argmin(errors)
            theta= read_pos()
            if min_index == 0:
                print 'slide_left_finger_down'
                #dtheta = -np.linalg.pinv(J_left) *  (e.reshape(e.shape[0], 1)) * dt
                #self.t2 = self.t2 + dtheta[0, 0]
                #self.translateLeft()
                #slide_left_finger_down(angle_conversion(self.t1,0))
                slide_left_finger_down(theta[0]-0.02)
            elif min_index == 1:
                print 'slide_left_finger_up'
                #dtheta = -np.linalg.pinv(J_left) *  (e.reshape(e.shape[0], 1)) * dt
                #self.t2 = self.t2 + dtheta[0, 0]
                #slide_left_finger_up(angle_conversion(self.t2,1))
                slide_left_finger_up(theta[1]-0.02)
            elif min_index == 2:
                print 'slide_right_finger_up'
                # dtheta = -np.linalg.pinv(J_right) * (e.reshape(e.shape[0], 1))* dt
                # self.t1 = self.t1 + dtheta[0, 0]
                # self.translateRight()
                # slide_right_finger_up(angle_conversion(self.t1,0))
                slide_right_finger_up(theta[0]-0.02)
            else:
                print 'slide_right_finger_down'
                # dtheta = -np.linalg.pinv(J_right) * (e.reshape(e.shape[0], 1))* dt
                # self.t1 = self.t1 + dtheta[0, 0]
                # self.translateRight()
                #slide_right_finger_up(angle_conversion(self.t2,1))
            ...
            dtheta_right = -np.linalg.pinv(J_right) *  (e.reshape(e.shape[0], 1)) * dt
            ...
            dtheta_left = -np.linalg.pinv(J_left) *  (e.reshape(e.shape[0], 1)) * dt
