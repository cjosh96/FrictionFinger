...
class visual_servoing:
	...
	def controller(self, goal_x,goal_y, goal_theta, TOLERANCE = 0.5):
		...
		while norm(e) > TOLERANCE:
			...
			dtheta_right = -Kp*np.linalg.pinv(J_right) *  (e.reshape(e.shape[0], 1))*dt_1
			...
			dtheta_left = -Kp*np.linalg.pinv(J_left) *  (e.reshape(e.shape[0], 1))*dt_1
			...
			print 'Cartesian Coordinates', self.x, self.y, 'theta1 = ', self.t1, 'theta2 = ', self.t2
			
			####### SET LIMITS FOR DELTA THETAS ########
			# if(abs(dtheta_left[0,0])<0.2):
			# 	dtheta_left[0,0] = np.sign(dtheta_left[0,0])*0.2

			# if(abs(dtheta_left[0,0])>=0.75):
				# dtheta_left[0,0] = np.sign(dtheta_left[0,0])*0.75

			# if(abs(dtheta_right[0,0])<0.2):
			# 	dtheta_right[0,0] = np.sign(dtheta_left[0,0])*0.2

			# if(abs(dtheta_right[0,0])>0.75):
				# dtheta_right[0,0] = np.sign(dtheta_left[0,0])*0.75
